...
class ContatoDAO
{
    ...
    public function recuperaTodosBarbeiros($stFiltro="")
    {
        require_once "ConexaoDB.php";        
        $db = new ConexaoDB();
        ...
        // Monta query Busca
        $sql = "SELECT contato.id, contato.nome, contato.sobrenome, contato.email, contato.telefone 
                FROM contato
                JOIN barbeiro
                  ON barbeiro.id = contato.id
                ".$stFiltro;
    ...
    public function cadastrarContatoDAO($contato) 
    {
        ...
        $sql = "INSERT INTO contato (nome, sobrenome, email, senha, telefone)
                VALUES 
                (?, ?, ?, ?, ?)";
